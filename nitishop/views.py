...
from .forms import EndOfDay, ExpenseAdd, DateChoose
...
def index(request):
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            return redirect('/gen_data/'+str(clean['this_year'])+'/'+str(clean['this_month'])+'/')
    else:
        form = DateChoose()
...
def generate_d(requset, year, month):
    t = Total.objects.filter(date__year=year,date__month=month)
    t = t.order_by('-date')
    return render(requset,'gen_data.html',{'month':switchcase(month),'data':t})